...
*   **Validación Generacional en el StateManager**: Para evitar colisiones tras el reciclaje de arenas, `ObtenerMetadatos(long tokenArena, int tamañoMinimo)` valida directamente que la arena esté activa (`ValueLINQArenaManager.IsArenaViva(tokenArena)`) y que la generación registrada en la tabla (`tabla.ArenaGen`) coincida exactamente con la del token de arena proporcionado. Asimismo, `ObtenerTabla(long token)` e `IntentarObtenerTabla(long token)` comprueban que la generación incrustada en el token de sesión coincida con la generación viva de la arena en `ValueLINQArenaManager.ObtenerGeneracion(idArena)` y en `tbl.ArenaGen`, lanzando `ValueLinqArenaInactivaException` ante cualquier discrepancia.
