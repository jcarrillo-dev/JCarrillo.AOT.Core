...
* Array `_indicesLibresStack` (`Arena/TablaSesiones.cs:23`): 4096 entradas * 4 bytes = 16.384 bytes (16 KB) (estimado).
...
* **Conservación a través de Particiones LIFO**: La estrategia de pila LIFO mitiga este desperdicio. Bajo baja concurrencia, solo se materializa una única partición: la de índice más alto (63), ya que la pila LIFO entrega primero los índices superiores (el constructor llena la pila con los índices 0..4095 y `PopIndice` extrae desde la cima, por lo que el primer índice entregado es el 4095, perteneciente a la partición 63; `Arena/TablaSesiones.cs:39,98-103`). Esto restringe la sobrecarga de relleno activa a $64 \times (24 \text{ bytes} + 60 \text{ bytes}) = 5.376 \text{ bytes}$ (~5,2 KB) (estimado). Las 63 particiones restantes permanecen sin materializar, ahorrando hasta $63 \times 8 \text{ KB} = 504 \text{ KB}$ de espacio en el heap en comparación con un modelo plano pre-asignado (estimado).
...
### 5.3 Lectura y Escritura Atómica de Tokens (Validación sin Bloqueo)
Además de los spinlocks, el sistema emplea un cuarto mecanismo de sincronización en las rutas calientes: el acceso atómico al campo `Token` de los metadatos mediante `TokenHelper.LeerToken` y `TokenHelper.EscribirToken` (`TokenHelper.cs:134-151`).
* **Ruta rápida en 64+ bits**: En procesos con punteros de 64 bits o más, las lecturas y escrituras de `long` son atómicas por hardware, por lo que se usa `Volatile.Read`/`Volatile.Write`. El coste es casi nulo: solo impone barreras de memoria (semántica acquire/release), sin instrucciones con prefijo `lock` ni contención entre núcleos (estimado).
* **Degradación en 32 bits**: En procesos de 32 bits, una lectura o escritura de 64 bits puede fragmentarse en dos operaciones de 32 bits, produciendo lecturas rasgadas (*torn reads*) si otro hilo escribe concurrentemente. Para prevenirlo, se degrada a `Interlocked.Read` e `Interlocked.Exchange`, que garantizan atomicidad a costa de operaciones de hardware más pesadas (estimado).
* **Selección cacheada de la arquitectura**: La rama se decide con el campo estático `Is64BitOMas` (`IntPtr.Size >= 8`, `TokenHelper.cs:11`), evaluado una única vez al cargar la clase en lugar de consultar `Environment.Is64BitProcess` en cada operación. Al ser un `static readonly bool`, el JIT puede eliminar la rama muerta por completo en el código generado (estimado).
* **Validación sin adquirir el spinlock del slot**: Esta ruta permite validar tokens sin contención de bloqueos. Por ejemplo, `TablaSesiones<T>.ObtenerMetadatos(long)` compara el token almacenado con el solicitado usando `TokenHelper.LeerToken` sin adquirir el spinlock del slot (`Arena/TablaSesiones.cs:190-191`), al igual que `IsMetadatoValido` (`Arena/TablaSesiones.cs:275`) y las comprobaciones de tokens de arena en el gestor (`ValueLINQArenaManager.cs:118-125`). Esto elimina la serialización en la ruta de lectura más frecuente: hasta 4096 hilos pueden validar tokens de slots distintos —o incluso del mismo slot— de forma simultánea sin girar (spin) sobre ningún bloqueo (estimado).
