...
* **Bits 36 - 63 (28 bits) - Versión de Sesión**: Rastreado localmente en cada slot. Se incrementa en cada operación de extracción (pop) (`ValueLINQConfig.VersionBits = 28`).
...
2. **Particionamiento de Índices**: El índice del slot se extrae a través de `TokenHelper.ObtenerSlotIndex(token)`. El índice de la partición se calcula desplazando el índice del slot 6 bits a la derecha (`SlotBits - SlotsParticionBits = 6`), y el offset del slot se resuelve mediante una operación AND a nivel de bit con `SlotsParticionMask` (63) (`Arena/TablaSesiones.cs:127-132`).
...
2. El token de sesión recién generado incrusta esta versión incrementada en sus 28 bits superiores (`TokenHelper.CrearToken`).
...
3. Los tokens de sesión caducados contienen la antigua generación de la arena. Durante la validación, el gestor verifica el estado de la arena mediante `ValueLINQArenaManager.IsArenaViva(long tokenArena)`. Dado que la generación almacenada en `_arenas[id].Token` es mayor que la generación codificada en el token de sesión antiguo, la validación falla.
4. Adicionalmente, liberar una arena limpia completamente las entradas de `TablaSesiones<T>` asociadas y establece sus tokens almacenados a `0L` (`ValueLINQArenaManager.cs:137-138`), invalidando todas las referencias.
...
2. **`ValueLinqArenaInactivaException`** (`Excepciones/ValueLinqArenaInactivaException.cs:13`):
   * *Condición de Disparo*: Lanzada cuando se intenta una asignación o acceso a sesión sobre un ID de arena que ha sido liberado o nunca fue asignado.
