...
3. Los tokens de sesión caducados contienen la antigua generación de la arena. Durante la validación, `ValueLINQStateManager<T>` solo comprueba que la arena con ese ID siga viva mediante la sobrecarga `ValueLINQArenaManager.IsArenaViva(int idArena)` (`ValueLINQStateManager.cs:91` y `:107`), que verifica que el token de arena almacenado sea distinto de `0L`, sin comparar generaciones (`ValueLINQArenaManager.cs:124-125`); si el ID aún no ha sido realquilado, esta comprobación falla y se lanza `ValueLinqArenaInactivaException`. Si el ID ya fue realquilado por una nueva arena, el rechazo ocurre en `TablaSesiones<T>.ObtenerMetadatos(long)`: la tabla recreada contiene en sus slots `0L` o tokens que incrustan la nueva Gen. de Arena (bits 24-35), por lo que la comparación de igualdad exacta `TokenHelper.LeerToken(ref metadatoRef.Token) != token` falla y se lanza `ValueLinqSesionExpiradaException` (`Arena/TablaSesiones.cs:190-191`). La sobrecarga `ValueLINQArenaManager.IsArenaViva(long tokenArena)` está reservada para tokens de ARENA —la usa `ValueLINQArena.IsViva` (`Arena/ValueLINQArena.cs:33`)— y compara el token completo por igualdad exacta (`==`), no por orden de generaciones (`ValueLINQArenaManager.cs:121-122`).
