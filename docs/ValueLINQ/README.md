...
- Esta configuración permite a los desarrolladores escribir consultas utilizando la sintaxis lambda convencional, eludiendo las advertencias continuas del compilador. Los materializadores del motor Delay (`ToList`, `ToArray`, `ToListStandard`, `ToArrayStandard` sobre `ValueLINQDelayStruct<T, TEnumerator>`) son **`public`** (disponibles mediante el espacio de nombres `JCarrillo.AOT.Core.Extensiones.ValueLINQ.Delay`), permitiendo materializar canalizaciones perezosas tanto en colecciones rentadas (`PooledList<T>`, `PooledArray<T>`, con 0 B de asignación persistente en heap) como en colecciones estándar administradas (`List<T>`, `T[]`, sujetas al diagnóstico `JCA0002`). Todos los materializadores garantizan la liberación determinista del enumerador subyacente mediante bloques `try-finally`. Adicionalmente, el consumidor puede optar por la enumeración directa (`foreach`) o el procesamiento en bloque (`ProcesarChunk`) para preservar la mínima latencia y el perfil de cero asignaciones en stack sin crear estructuras intermedias.
