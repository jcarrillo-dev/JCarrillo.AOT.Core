...
namespace JCarrillo.AOT.Core.ValueLINQ.Excepciones
{
    ...
    public sealed class ValueLinqArenaCruzadaException(int arenaEsperada, int arenaEncontrada) : InvalidOperationException($"Operación inválida en ValueLINQ: acceso cruzado entre arenas. Se esperaban todos los operandos en la arena [{arenaEsperada}] (la del primer elemento), pero uno pertenece a la arena [{arenaEncontrada}]. No se pueden combinar sesiones de arenas distintas.")
    {
        /// <summary>
